...
##################################################################
#
# Processing Script for Student-Enrollment-by-Race-Ethnicity
...
for (i in 4:length(student_enrollment_dist_noTrend)) {
...
cols_to_stack <- c("American Indian or Alaska Native",
                   "Asian",
...
test <- complete_student_enrollment_long[,c("District", "Year", "Race/Ethnicity")]
test2<-test[duplicated(test), ]

#Write CSV
write.table(
  complete_student_enrollment_long,
  file.path(path_to_top_level, "data", "student_enrollment_by_race_ethnicity_2011-2016.csv"),
  sep = ",",
  row.names = F
)